...
class FlagshipConfig:
    ...
    activation_mode: str = "continuous"
...
class FlagshipRuntime:
    ...
    def _handle_runtime_command(self, text: str) -> core.AssistantReply | None:
        ...
        if core.contains_any(text, ("выключи звук", "режим тишины", "тихий режим", "без озвучки")):
            self.tts.enabled = False
            return core.AssistantReply("Озвучка отключена.", should_speak=False)

        if core.contains_any(text, ("включи звук", "включи озвучку", "голосовой режим")):
...
def main() -> int:
    ...
    if ui_mode == "gui":
        try:
            ...
            logging.warning("GUI недоступен, переключаюсь на console: %s", error)
